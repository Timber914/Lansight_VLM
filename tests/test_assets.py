...
class TestAssets(unittest.TestCase):
    ...
    def test_weights_present(self):
        ...
        # 2) 项目 out/transformers/LanSight2-V 中的 transformers 权重
        # 3) 兼容旧路径：assets/{torch_weights,transformers}
        out_pth = list((ROOT/'out').glob('*.pth'))
        out_trf = ROOT/'out'/'transformers'/'LanSight2-V'/'pytorch_model.bin'
        assets_pth = list((ROOT/'assets'/'torch_weights').glob('*.pth'))
        assets_trf = ROOT/'assets'/'transformers'/'LanSight2-V'/'pytorch_model.bin'
